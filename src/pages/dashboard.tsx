...
import {
  ...
  useDonorProfileCount,
  useDonorshipApplicationCount,
  useNonProfitOrganizationCount,
} from 'lib/roq';

import { ApplicationStatusListPage as ShowCaseList } from 'pages/application-statuses';

function DashboardPage() {
  const { session } = useSession();

  const donorProfileStat = useDonorProfileCount();

  const donorshipApplicationStat = useDonorshipApplicationCount();

  const nonProfitOrganizationStat = useNonProfitOrganizationCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Donor Profiles',
      result: donorProfileStat?.data,
      isLoading: donorProfileStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Donorship Applications',
      result: donorshipApplicationStat?.data,
      isLoading: donorshipApplicationStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Non Profit Organizations',
      result: nonProfitOrganizationStat?.data,
      isLoading: nonProfitOrganizationStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Application Statuses
                </Text>
                <NextLink href={`application-statuses`} passHref legacyBehavior>
                  <Button
